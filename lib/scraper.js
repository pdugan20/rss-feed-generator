...
class Scraper {
    ...
    try {
      ...
      if (domain.includes('seattletimes.com')) {
        ...
        if (articles.length === 0) {
          $(
            '[class*="results-story"], [class*="story-list"] article, article[class*="story"]'
          ).each((index, element) => {
            if (articles.length >= 20) return false;
        ...
        $(selectors.article).each((index, element) => {
          ...
          const title =
            $article.find(selectors.title).first().text().trim() ||
            $article.find('h1, h2, h3, h4').first().text().trim();

          const link = this.resolveUrl(
            $article.find(selectors.link).attr('href') || $article.find('a').first().attr('href'),
            url
          );

          const description =
            $article.find(selectors.description).text().trim() ||
          ...
          const imageUrl = this.resolveUrl(
            $article.find(selectors.image).attr('src') || $article.find('img').first().attr('src'),
            url
          );
  ...
  getSelectorsForDomain(domain) {
    const selectorMap = {
      'seattletimes.com': {
        article:
          '[data-testid="story-list"] article, main article, .story-list article, [class*="StoryCard"], [class*="story-card"], .package-item',
